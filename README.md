...
| **Numbers** | every count above is read from `docs/FACTS.json` and re-checked in CI — a README that drifts from the suite fails the build |

> **The counts above are not typed by hand.** Every number a document in this repository
> claims lives in `docs/FACTS.json` first, and `npm run check:conventions` compares it against the
> actual suite on every push (`.github/workflows/api-tests.yml`). Counts cover git-tracked files
> only, so the check gives the same answer in CI and on any machine. A stale README is a red
> build, not a thing you find out in an interview.
