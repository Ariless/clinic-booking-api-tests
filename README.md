...
**Playwright (TypeScript)** automation for a clinic booking API.
...
> machine (`pending → confirmed → completed`, with `422 INVALID_TRANSITION` on anything else).
 Controlled **SUT**: REST + demo UI under `public/`. Same engineering habits as a production-grade framework: **POM**, **API client layer**, **tagged suites**, **AJV schema validation**, **no sleeps**, **`data-qa`** selectors.
