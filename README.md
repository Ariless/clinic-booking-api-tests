...
- **`docs/KNOWN_ISSUES.md`** — bug register: **28 closed entries** — product bugs (IDOR, a11y, WS `ClinicCore`, banner timing, reschedule 409 with an active waitlist, the three waitlist-offer defects the invariant contract surfaced), CI and environment issues (route pattern, private repo auth, SQLite readonly in Docker, teardown cascades) suite defects (`TST-01…TST-04`) and the `isAvailable` serialisation mismatch the mobile pact verification caught (B-15); **1 open bug** (procedure booked into a slot shorter than its required duration); **3 design debt items** + 1 dead code finding (unreachable `INVALID_PATTERN` errorCode); each entry with business impact, severity, how found, fix applied.
