...
- Name: api/auth.delete.test.ts >> account soft delete @api >> POST /auth/refresh — 401 refresh token rejected after account deletion @api @security
- Location: tests/api/auth.delete.test.ts:43:7

# Error details

```
TypeError: auth.refresh is not a function
```
