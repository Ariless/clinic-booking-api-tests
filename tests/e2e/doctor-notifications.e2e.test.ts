...
test("patient books via API — doctor appointments page shows booking toast without reload @e2e", async ({ page, request, user, slot, loginPage, doctorAppointmentsPage: doctorApptPage }) => {
...
test("patient cancels via API — doctor appointments page shows cancellation toast without reload @e2e", async ({ page, request, user, slot, loginPage, doctorAppointmentsPage: doctorApptPage }) => {
    const { slot: slotBody } = slot;
    const appointments = new AppointmentsClient(request);
    const patientAuth = { headers: { Authorization: `Bearer ${user.token}` } };
