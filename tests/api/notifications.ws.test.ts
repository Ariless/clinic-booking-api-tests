...
// To run: npx playwright test notifications.ws.test.ts
// SUT must be running (standard: npm run dev — no extra env vars needed)

const BASE_URL = process.env.BASE_URL || 'http://localhost:3000';
const WS_URL = BASE_URL.replace(/^http/, 'ws') + '/ws';

function connectDoctor(token: string): Promise<WebSocket> {
...
test.describe("notifications — WebSocket delivery @ws", () => {
    test("POST /api/v1/appointments — doctor WS receives appointment.booked when patient books @ws", async ({ request, user, slot }) => {
    ...
    test("PATCH /api/v1/appointments/:id/cancel — doctor WS receives appointment.cancelled_by_patient @ws", async ({ request, user, slot }) => {
        const { slot: slotBody, doctorToken } = slot;
        const patientAuth = { headers: { Authorization: `Bearer ${user.token}` } };
        const appointments = new AppointmentsClient(request);
    ...
    test("WS connect with invalid token — connection rejected with code 4001 @ws", async () => {
