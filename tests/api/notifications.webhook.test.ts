...
// 2. Run tests:  WEBHOOK_URL=http://localhost:9001 npx playwright test notifications.webhook.test.ts
// Note: appointment.confirmed is covered in tests/e2e/booking.cross-layer.test.ts
...
test.describe("notifications — webhook delivery @webhook", () => {
    test.skip(
        !WEBHOOK_CONFIGURED,
        "Restart SUT with WEBHOOK_URL=http://localhost:9001 npm run dev, then: WEBHOOK_URL=http://localhost:9001 npx playwright test notifications.webhook.test.ts"
    );

    test("PATCH /api/v1/appointments/:id/reject — webhook fires appointment.rejected @webhook", async ({ request, user, slot }) => {
    ...
    test("PATCH /api/v1/appointments/:id/cancel — webhook fires appointment.cancelled_by_patient @webhook", async ({ request, user, slot }) => {
    ...
    test("PATCH /api/v1/appointments/:id/cancel-as-doctor — webhook fires appointment.cancelled_by_doctor @webhook", async ({ request, user, slot }) => {
        const { slot: slotBody, doctorToken } = slot;
        const patientAuth = { headers: { Authorization: `Bearer ${user.token}` } };
        const doctorAuth = { headers: { Authorization: `Bearer ${doctorToken}` } };
        const appointments = new AppointmentsClient(request);
