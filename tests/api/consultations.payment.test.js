...
const { ConsultationsClient } = require("../../api/ConsultationsClient");
const { dbClient } = require("../../utils/dbClient");

// To run:
// Feature flag:  npx playwright test consultations.payment.test.js (default PAYMENT_MODE=disabled)
// Happy path:    PAYMENT_MODE=mock_success npx playwright test consultations.payment.test.js
// Failure path:  PAYMENT_MODE=mock_fail    npx playwright test consultations.payment.test.js
...
test.describe("consultations — payment mock_success @payment", () => {
    test.skip(
        ...
        "Restart SUT with PAYMENT_MODE=mock_success npm run dev, then: PAYMENT_MODE=mock_success npx playwright test consultations.payment.test.js"
    );
    ...
    test("GET /api/v1/consultations/me — 200 returns created consultation @payment", async ({ request, user }) => {
        ...
        const found = body.find((c) => c.id === created.consultation.id);
...
test.describe("consultations — payment mock_fail @payment", () => {
    test.skip(
        ...
        "Restart SUT with PAYMENT_MODE=mock_fail npm run dev, then: PAYMENT_MODE=mock_fail npx playwright test consultations.payment.test.js"
    );
