...
test("POST /api/v1/appointments — 429 RATE_LIMITED after exhausting per-IP booking limit @rate-limit", async ({ request, user }) => {
    test.skip(BOOKING_MAX > 5, `RATE_LIMIT_BOOKING_MAX=${BOOKING_MAX}; set RATE_LIMIT_BOOKING_MAX=2 and RATE_LIMIT_BOOKING_WINDOW_MS=5000 to run`);
